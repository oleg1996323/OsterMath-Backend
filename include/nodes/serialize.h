...
class SerialData{
    ...
    void insert_node(uint64_t id, AbstractNode* node){
        if(node)
            nodes_[id]=node;
    }
    ...
    AbstractNode* get_node(uint64_t hash){
    ...
    void insert_var(uint64_t var_id, const VariableNode* node) noexcept;
    ...
    std::unordered_map<uint64_t,AbstractNode*> nodes_;
