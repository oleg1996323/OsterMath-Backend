...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        ...
        if (sheet.GetCell(*cell_) == nullptr) 
            const_cast<SheetInterface&>(sheet).SetCell(*cell_, ""s);
        ...
        if (std::holds_alternative<std::string>(value)) {
            if(std::get<std::string>(value).empty())
                return 0.;
            if(std::all_of(std::get<std::string>(value).begin(),std::get<std::string>(value).end(),[](const char& ch){
