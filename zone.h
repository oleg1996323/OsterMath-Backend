...
struct ZoneBounds{
    Bound::TempName left_;
    Bound::TempName right_;
...
class SubZone{
    ...
    SubZone(ZoneNum zone, SubZoneName subzone):zone_({zones_.at(zone).at(subzone)}){}
    ...
    bool operator==(const SubZone& other){
        return zone_==other.zone_ && p_min_==other.p_min_ && p_max_==other.p_max_;
    }

    struct hash{
        size_t operator()(const SubZone& zone){
...
class Zone{
    ...
    std::unordered_set<SubZone, SubZone::hash> sub_zones_;
